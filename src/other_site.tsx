...
function OtherSite(){
    ...
    const createMessiahPressed = async()=>{
        ...
        await deployMessiahSystem(createMessiahProps.erc721Add, createMessiahProps.erc20Add);
    ...
    const CreateMessiahHandleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const createMessiahForm = ()=>{
        if(!checkFlag){
            return(
                <div>
                    <Paper>
                        <Box sx={{m:2}}>
                            <Typography variant="h4" gutterBottom>You Can Check Messiah Exists</Typography>
                        <Box sx={{m:4}}>
                                <Typography variant="h5" gutterBottom>Please Input ERC-721 Token Address</Typography>
                                <TextField
                                type="text"
                                ...
                                />
                                <Box sx={{mt:2}}>
                                    <Button sx={{mb:2, backgroundColor:"mediumblue", "&:hover": {background: "blueviolet"}}} variant="contained" 
                                    onClick={()=>{checkMessiahExistsPressed()}} fullWidth>Check Messiah Exists</Button>
                                </Box>
                            </Box>
                        </Box>
                    </Paper>
        ...
        if(messiahExists){
            return(
                <Typography variant="h4" gutterBottom>You Go Next Step</Typography>
            );
        }
        else{
            return(
            <div>
                  <Paper>
                        <Box sx={{m:2}}>
                            <Typography variant="h4" gutterBottom>You can make Messiah</Typography>
                        <Box sx={{m:4}}>
                            <Typography variant="h5" gutterBottom>Please Input ERC-721 Token Address</Typography>
                            <TextField
                            type="text"
                        ...
                        <Box sx={{m:4}}>
                            <Typography variant="h5" gutterBottom>Please Input ERC-20 Token Address</Typography>
                            <TextField
                            type="text"
                            name="erc20Add"
                        ...
                        <Box sx={{mt:2}}>
                            <Button sx={{mb:2, backgroundColor:"mediumblue", "&:hover": {background: "blueviolet"}}} variant="contained"
                            onClick={()=>{createMessiahPressed()}} fullWidth>Create Messiah System</Button>
                        </Box>
                        </Box>
                    </Paper>
    ...
    return(
        <div>
            <Grid container alignItems="center" justifyContent="center">
                <Box mt={5} mb={5}>
                    <Typography variant="h2" gutterBottom component="div">Messiah System</Typography>
                </Box>
            </Grid>
